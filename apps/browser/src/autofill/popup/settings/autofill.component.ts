...
export class AutofillComponent implements OnInit {
  ...
  /**
   * Reactive form that binds the two inline-menu password-generator disable settings:
   *
   * - `disableInlineFillAutogeneratedPassword`: when `true`, the "Fill autogenerated
   *   password" popup is **never** shown when a password field is focused.
   * - `disableInlineFillAutogeneratedPasswordWhenSiteExists`: when `true`, the popup
   *   is suppressed only when the current site already has a saved Login in the vault;
   *   for sites with no vault entry the popup is still shown.
   *
   * Both controls are initialized in `ngOnInit` with the current persisted values and
   * write back to {@link AutofillSettingsServiceAbstraction} on every change.
   */
  protected inlineMenuPasswordGeneratorForm = new FormGroup({
    disableInlineFillAutogeneratedPassword: new FormControl(),
    disableInlineFillAutogeneratedPasswordWhenSiteExists: new FormControl(),
  });
  ...
  async ngOnInit() {
    ...
    /** Inline menu password generator settings
     *
     * Load the current persisted values from storage, patch them into the form
     * without triggering change events (to avoid spurious writes on init), then
     * subscribe to subsequent changes and persist them immediately.
     */

    const disableInlineFillAutogeneratedPassword = await firstValueFrom(
      this.autofillSettingsService.disableInlineFillAutogeneratedPassword$,
    );
    this.inlineMenuPasswordGeneratorForm.controls.disableInlineFillAutogeneratedPassword.patchValue(
      disableInlineFillAutogeneratedPassword,
      { emitEvent: false },
    );

    const disableInlineFillAutogeneratedPasswordWhenSiteExists = await firstValueFrom(
      this.autofillSettingsService.disableInlineFillAutogeneratedPasswordWhenSiteExists$,
    );
    this.inlineMenuPasswordGeneratorForm.controls.disableInlineFillAutogeneratedPasswordWhenSiteExists.patchValue(
      disableInlineFillAutogeneratedPasswordWhenSiteExists,
      { emitEvent: false },
    );

    // Persist Setting 1 whenever the checkbox value changes.
    this.inlineMenuPasswordGeneratorForm.controls.disableInlineFillAutogeneratedPassword.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((value) => {
        void this.autofillSettingsService.setDisableInlineFillAutogeneratedPassword(value);
      });

    // Persist Setting 2 whenever the checkbox value changes.
    this.inlineMenuPasswordGeneratorForm.controls.disableInlineFillAutogeneratedPasswordWhenSiteExists.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((value) => {
        void this.autofillSettingsService.setDisableInlineFillAutogeneratedPasswordWhenSiteExists(
          value,
        );
      });
