...
describe("OverlayBackground", () => {
  ...
  /** Mock observable for the "Disable Inline Fill Autogenerated Password" setting. */
  let disableInlineFillAutogeneratedPasswordMock$: BehaviorSubject<boolean>;
  /** Mock observable for the "Disable Inline Fill Autogenerated Password When Site Exists" setting. */
  let disableInlineFillAutogeneratedPasswordWhenSiteExistsMock$: BehaviorSubject<boolean>;
  ...
  beforeEach(() => {
    ...
    // Wire the two new disable-settings observables so tests can toggle them per-test.
    disableInlineFillAutogeneratedPasswordMock$ = new BehaviorSubject(false);
    disableInlineFillAutogeneratedPasswordWhenSiteExistsMock$ = new BehaviorSubject(false);
    autofillSettingsService.disableInlineFillAutogeneratedPassword$ =
      disableInlineFillAutogeneratedPasswordMock$;
    autofillSettingsService.disableInlineFillAutogeneratedPasswordWhenSiteExists$ =
      disableInlineFillAutogeneratedPasswordWhenSiteExistsMock$;
  ...
  describe("handle port onConnect", () => {
    ...
    /**
     * Tests for shouldInitInlineMenuPasswordGenerator with the two new user settings.
     *
     * The helper sets up a focused password field (account-creation path) and an
     * unlocked vault before each test, then manipulates the settings mock to verify
     * that the correct suppression logic fires.
     */
    describe("shouldInitInlineMenuPasswordGenerator — disable settings", () => {
      ...
      it("does NOT generate a password when disableInlineFillAutogeneratedPassword is true", async () => {
        // Setting 1 enabled → popup suppressed unconditionally.
        disableInlineFillAutogeneratedPasswordMock$.next(true);
      ...
      it("does NOT generate a password when disableInlineFillAutogeneratedPasswordWhenSiteExists is true and a Login cipher exists for the site", async () => {
        // Setting 2 enabled and vault returns a matching Login cipher.
        disableInlineFillAutogeneratedPasswordWhenSiteExistsMock$.next(true);
      ...
      it("DOES generate a password when disableInlineFillAutogeneratedPasswordWhenSiteExists is true but no Login cipher exists for the site", async () => {
        // Setting 2 enabled, but vault has no saved Login for this URL → popup shown.
        disableInlineFillAutogeneratedPasswordWhenSiteExistsMock$.next(true);
        cipherService.getAllDecryptedForUrl.mockResolvedValue([]);
      ...
      it("does NOT suppress the popup when the only vault ciphers for the site are non-Login types (e.g. Card)", async () => {
        // Setting 2 enabled, but the vault only has a Card cipher (not a Login) → popup shown.
        disableInlineFillAutogeneratedPasswordWhenSiteExistsMock$.next(true);
      ...
      it("DOES generate a password when both disable settings are false (default behavior)", async () => {
        // Both settings off → normal behavior, popup and generator shown.
        disableInlineFillAutogeneratedPasswordMock$.next(false);
        disableInlineFillAutogeneratedPasswordWhenSiteExistsMock$.next(false);
      ...
      it("Setting 1 takes priority: does NOT generate a password even when no Login cipher exists for the site", async () => {
        // Setting 1 (unconditional disable) overrides Setting 2 logic.
        disableInlineFillAutogeneratedPasswordMock$.next(true);
        disableInlineFillAutogeneratedPasswordWhenSiteExistsMock$.next(true);
        cipherService.getAllDecryptedForUrl.mockResolvedValue([]); // no vault entry

        await initOverlayElementPorts();
        await flushPromises();

        // Vault should never be queried because Setting 1 already returned false.
        expect(cipherService.getAllDecryptedForUrl).not.toHaveBeenCalled();
        expect(generatorService.generate$).not.toHaveBeenCalled();
      });
    }); // end describe("shouldInitInlineMenuPasswordGenerator — disable settings")
  }); // end describe("handle port onConnect")
