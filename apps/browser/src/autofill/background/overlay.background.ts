...
export class OverlayBackground implements OverlayBackgroundInterface {
   ...
   *   3. If the user has enabled **Disable Inline Fill Autogenerated Password** the
   *      popup is suppressed unconditionally — `false` is returned immediately.
   *   4. If the user has enabled **Disable Inline Fill Autogenerated Password when
   *      Password for Site Exists in Vault**, the vault is queried for Login ciphers
   *      that match the current tab URL.  If at least one match is found the popup
   *      is suppressed so the existing vault entry's fill behavior takes over;
   *      otherwise the popup is shown so the user can generate a password for the
   *      new account.
   *   5. If no credential has been generated yet, one is requested and awaited so
  ...
  private async shouldInitInlineMenuPasswordGenerator(
    ...
    // Check Setting 1: "Disable Inline Fill Autogenerated Password".
    // When enabled, suppress the popup entirely regardless of vault contents.
    const disableAutogeneratedPassword = await firstValueFrom(
      this.autofillSettingsService.disableInlineFillAutogeneratedPassword$,
    );
    if (disableAutogeneratedPassword) {
      return false;
    }

    // Check Setting 2: "Disable Inline Fill Autogenerated Password when Password for Site Exists".
    // When enabled, query the vault for Login ciphers that match the current URL.
    // If a match exists, suppress the popup so the vault-configured fill behavior applies.
    // If no match exists, fall through and show the generator for the new account.
    const disableWhenSiteExists = await firstValueFrom(
      this.autofillSettingsService.disableInlineFillAutogeneratedPasswordWhenSiteExists$,
    );
    if (disableWhenSiteExists && tab?.url) {
      const activeUserId = await firstValueFrom(
        this.accountService.activeAccount$.pipe(getOptionalUserId),
      );
      if (activeUserId) {
