...
/**
 * Persists the user's chosen behavior for the inline autogenerated password popup.
 * Defaults to "normal" (the original extension behavior — popup always shown).
 */
const INLINE_MENU_PASSWORD_GENERATOR_BEHAVIOR = new UserKeyDefinition<InlineMenuPasswordGeneratorBehavior>(
  AUTOFILL_SETTINGS_DISK,
  "inlineMenuPasswordGeneratorBehavior",
  {
    deserializer: (value: InlineMenuPasswordGeneratorBehavior) =>
...
export abstract class AutofillSettingsServiceAbstraction {
  ...
  /** Emits the user's chosen inline menu password generator behavior. */
  inlineMenuPasswordGeneratorBehavior$: Observable<InlineMenuPasswordGeneratorBehavior>;
  /** Persists the {@link inlineMenuPasswordGeneratorBehavior$} setting. */
  setInlineMenuPasswordGeneratorBehavior: (
    newValue: InlineMenuPasswordGeneratorBehavior,
  ) => Promise<void>;
}
...
export class AutofillSettingsService implements AutofillSettingsServiceAbstraction {
  ...
  private inlineMenuPasswordGeneratorBehaviorState: ActiveUserState<InlineMenuPasswordGeneratorBehavior>;
  readonly inlineMenuPasswordGeneratorBehavior$: Observable<InlineMenuPasswordGeneratorBehavior>;
  ...
  constructor(
    ...
    this.inlineMenuPasswordGeneratorBehaviorState = this.stateProvider.getActive(
      INLINE_MENU_PASSWORD_GENERATOR_BEHAVIOR,
    );
    this.inlineMenuPasswordGeneratorBehavior$ =
      this.inlineMenuPasswordGeneratorBehaviorState.state$.pipe(
        map((x) => x ?? InlineMenuPasswordGeneratorBehavior.Normal),
      );
  }
  ...
  async setInlineMenuPasswordGeneratorBehavior(
    newValue: InlineMenuPasswordGeneratorBehavior,
  ): Promise<void> {
    await this.inlineMenuPasswordGeneratorBehaviorState.update(() => newValue);
  }
}
