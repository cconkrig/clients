...
/**
 * Persists whether the user has opted to completely disable the inline autogenerated
 * password popup across all sites. When `true`, the "Fill autogenerated password"
 * suggestion is never shown when a password field receives focus, regardless of
 * whether any vault entry exists for the current site.
 *
 * Default: `false` (popup is shown normally).
 */
const DISABLE_INLINE_FILL_AUTOGENERATED_PASSWORD = new UserKeyDefinition(
  AUTOFILL_SETTINGS_DISK,
...
/**
 * Persists whether the user has opted to suppress the inline autogenerated password
 * popup only when a saved login already exists in the vault for the current site.
 *
 * When `true`:
 *   - If a `Login` cipher matching the current URL is found in the vault, the
 *     autogenerated password popup is hidden and the normal vault-configured
 *     autofill behavior for that site takes effect instead.
 *   - If no matching `Login` cipher exists, the popup is shown as usual so the
 *     user can still generate a strong password for a new account.
 *
 * Default: `false` (popup is shown regardless of vault contents).
 */
const DISABLE_INLINE_FILL_AUTOGENERATED_PASSWORD_WHEN_SITE_EXISTS = new UserKeyDefinition(
  AUTOFILL_SETTINGS_DISK,
...
export abstract class AutofillSettingsServiceAbstraction {
  ...
  /**
   * Emits `true` when the user has enabled "Disable Inline Fill Autogenerated Password".
   * While `true`, the autogenerated password popup is suppressed on every password field focus.
   */
  disableInlineFillAutogeneratedPassword$: Observable<boolean>;
  /** Persists the {@link disableInlineFillAutogeneratedPassword$} setting. */
  setDisableInlineFillAutogeneratedPassword: (newValue: boolean) => Promise<void>;
  /**
   * Emits `true` when the user has enabled "Disable Inline Fill Autogenerated Password
   * when Password for Site Exists in Vault".
   * While `true`, the popup is only suppressed for sites that already have a saved Login cipher.
   */
  disableInlineFillAutogeneratedPasswordWhenSiteExists$: Observable<boolean>;
  /** Persists the {@link disableInlineFillAutogeneratedPasswordWhenSiteExists$} setting. */
  setDisableInlineFillAutogeneratedPasswordWhenSiteExists: (newValue: boolean) => Promise<void>;
}
...
export class AutofillSettingsService implements AutofillSettingsServiceAbstraction {
  ...
  private disableInlineFillAutogeneratedPasswordState: ActiveUserState<boolean>;
  readonly disableInlineFillAutogeneratedPassword$: Observable<boolean>;

  private disableInlineFillAutogeneratedPasswordWhenSiteExistsState: ActiveUserState<boolean>;
  readonly disableInlineFillAutogeneratedPasswordWhenSiteExists$: Observable<boolean>;
  ...
  constructor(
    ...
    this.disableInlineFillAutogeneratedPasswordState = this.stateProvider.getActive(
      DISABLE_INLINE_FILL_AUTOGENERATED_PASSWORD,
    );
    this.disableInlineFillAutogeneratedPassword$ =
      this.disableInlineFillAutogeneratedPasswordState.state$.pipe(map((x) => x ?? false));

    this.disableInlineFillAutogeneratedPasswordWhenSiteExistsState = this.stateProvider.getActive(
      DISABLE_INLINE_FILL_AUTOGENERATED_PASSWORD_WHEN_SITE_EXISTS,
    );
    this.disableInlineFillAutogeneratedPasswordWhenSiteExists$ =
      this.disableInlineFillAutogeneratedPasswordWhenSiteExistsState.state$.pipe(
        map((x) => x ?? false),
      );
  }
  ...
  /**
   * Persists the user's choice to unconditionally disable the inline autogenerated
   * password popup. When `newValue` is `true` the popup will never appear on any
   * password field, regardless of vault contents.
   */
  async setDisableInlineFillAutogeneratedPassword(newValue: boolean): Promise<void> {
    await this.disableInlineFillAutogeneratedPasswordState.update(() => newValue);
  }

  /**
   * Persists the user's choice to suppress the inline autogenerated password popup
   * only when a saved Login cipher already exists for the current site. When
   * `newValue` is `true` the popup is hidden for known sites and shown for new ones.
   */
  async setDisableInlineFillAutogeneratedPasswordWhenSiteExists(newValue: boolean): Promise<void> {
    await this.disableInlineFillAutogeneratedPasswordWhenSiteExistsState.update(() => newValue);
  }
}
